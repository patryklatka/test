...
# Adresy MQTT i dane połączenia
mqtt_broker = "1855d1e75c264a00b0fdffc55e0ec025.s1.eu.hivemq.cloud"
mqtt_port = 8883
mqtt_topic = "sensor/data"

# Zmienna do przechowywania ostatnich danych z sensora
last_data = None
...
def on_message(client, userdata, msg):
    global last_data
    last_data = json.loads(msg.payload)  # Załóżmy, że dane są w formacie JSON
    print(f"Received message: {last_data}")
    socketio.emit('new_data', last_data)  # Wyślij dane do frontend-u

# Funkcja do uruchomienia klienta MQTT
def start_mqtt():
    client = mqtt.Client()
    client.on_connect = on_connect
    client.on_message = on_message
